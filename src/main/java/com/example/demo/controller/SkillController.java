...
public class SkillController {
    ...
    public ResponseEntity<Skill> create(@Valid @RequestBody Skill s) throws IdInvadateException {
        if (s.getName() != null && this.skillService.isNameExist(s.getName())) {
            throw new IdInvadateException("skill name" + s.getName() + "đã tồn tại");
        }
    ...
    public ResponseEntity<Skill> update(@Valid @RequestBody Skill s) throws IdInvadateException {
        Skill currenSkill = this.skillService.fetchSkillById(s.getId());
        if (currenSkill == null) {
            throw new IdInvadateException("skill id" + s.getId() + "không tồn tại");
        }
